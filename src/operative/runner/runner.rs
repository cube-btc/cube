...
pub async fn run(
    ...
    // 10.d Initialize params manager.
    let params_manager: PARAMS_MANAGER = match ParamsManager::new(chain) {
        Ok(params_manager) => params_manager,
        Err(err) => {
            println!("{} {:?}", "Error initializing params manager: ".red(), err);
            return;
        }
    };
