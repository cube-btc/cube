...
impl SessionPool {
    ...
    pub async fn exec_deploy_in_pool(
        &mut self,
        deploy: &Deploy,
        ...
        match self.state {
            ...
            _ => {
                if self.added_entries.len() >= MAX_IN_POOL_ENTRIES {
                    return Err(ExecDeployInPoolError::PoolOverloadedError);
                }
            }
        };
        ...
        let (batch_height, batch_timestamp, _) = self
            .batch_info
            .ok_or(ExecDeployInPoolError::BatchInfoNotFoundError)?;

        deploy
            .bls_verify(deploy_bls_signature)
            .map_err(|err| ExecDeployInPoolError::DeployBLSVerifyError(format!("{err:?}")))?;
        ...
        {
            return Err(ExecDeployInPoolError::DeployValidateTargetError {
                targeted_at_batch_height,
                execution_batch_height,
            });
        }
        ...
        match deploy_result {
            Ok(deploy_entry) => {
                ...
                let entry_id = deploy_entry
                    .entry_id(batch_height, entry_index_in_batch)
                    .ok_or(ExecDeployInPoolError::EntryIdDerivationError)?;
                self.added_entries.push(deploy_entry.clone());
                self.added_individual_entry_bls_signatures
