...
pub async fn run(
    ...
    let operating_kind = OperatingKind::Engine;
    ...
    // #10 Check if this is the engine.
    if key_holder.secp_public_key_point().serialize_xonly() != engine_key(chain) {
        eprintln!("{}", "Engine <nsec> does not match.".red());
        return;
    }
...
pub async fn cli(peer_manager: &mut PEER_MANAGER) {
    ...
    for line in handle.lines() {
        ...
        match parts[0] {
            // Main commands:
            "exit" => break,
            "clear" => ecli::clear::clear_command(),
            "ops" => ecli::ops::ops_command(peer_manager).await,
