...
use crate::communicative::tcp::protocol::deploy::{
    DeployRequestBody, DeployResponseBody, DeployResponseError,
};
...
pub async fn handle_deploy_request(
    timestamp: i64,
    payload: &[u8],
    ...
    let DeployRequestBody {
        ...
        Some(req) => req,
        None => {
            let body = DeployResponseBody::err(DeployResponseError::DeserializeDeployRequestError);
    ...
    let mut response: Option<DeployResponseBody> = None;
    for attempt in 1..=MAX_EXEC_ATTEMPTS {
        ...
        match attempt_result {
            Ok((entry_id, entry, batch_height, batch_timestamp)) => {
                response = Some(DeployResponseBody::ok(
                    entry_id,
                    batch_height,
                    batch_timestamp,
                    entry,
            ...
            Err(err) => {
                let retry_after_settle = matches!(
                    err,
                    ExecDeployInPoolError::SessionInactiveError
                        | ExecDeployInPoolError::SessionBreakError
                );
                ...
                response = Some(DeployResponseBody::err(
                    DeployResponseError::ExecDeployInPoolError(err),
                ));
                break;
            }
        }
    ...
    let response_package = TCPPackage::new(PackageKind::DeployProtocol, timestamp, &response_bytes);
