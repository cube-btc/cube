...
impl Liftup {
    ...
    pub async fn decode_ape(
        ...
        let root_account: RootAccount = RootAccount::decode_ape(
            bit_stream,
        ...
        // 4 Collect one-bit lift kind tags (0 => v1, 1 => v2).
        ...
        // 5 Resolve the outpoints in the UTXO set and construct the lifts.
        ...
        let mut lift_prevtxos = Vec::<Lift>::with_capacity(number_of_lifts as usize);
        {
            ...
            for (outpoint, is_v2_lift) in collected_outpoints.into_iter().zip(lift_kind_bits) {
                let txout = _utxo_set
                    .txout_by_outpoint(&outpoint)
                    .ok_or(LiftupAPEDecodeError::UnableToLocateLiftOutpointInUTXOSetError(
                        outpoint,
                    ))?;
        ...
        // 6 Construct and return the decoded `Liftup`.
        let liftup = Liftup::new(root_account, lift_prevtxos);

        // 7 Return the decoded `Liftup`.
        Ok(liftup)
    }
}