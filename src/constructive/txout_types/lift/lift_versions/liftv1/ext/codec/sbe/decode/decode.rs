...
/// Decodes the shared lift payload after the variant byte (v1, v2, or [`Lift::Unknown`](crate::constructive::txo::lift::lift::Lift)): keys, outpoint, one `TxOut`.
pub(crate) fn decode_lift_v1_sbe_body(
    payload: &[u8],
    ...
    if total < 32 {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForAccountKey { got_total: total },
        );
    }
    ...
    if rest.len() < 32 {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForEngineKey {
                got_total: rest.len(),
            },
        );
    }
    ...
    if rest.len() < 36 {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForOutPoint {
                got_total: rest.len(),
            },
        );
    }
    ...
    if rest.len() < 8 {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForTxOutValue {
                got_total: rest.len(),
            },
        );
    }
    ...
    if rest.is_empty() {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForTxOutScriptLengthPrefix {
                got_total: 0,
            },
        );
    }
    ...
    if rest.len() < script_len {
        return Err(
            LiftV1SBEDecodeError::LiftV1SBEInsufficientBytesForTxOutScriptPayload {
                got_total: rest.len(),
                script_len,
            },
        );
    ...
    let txout = TxOut::from_bytes(&txout_bytes)
        .ok_or(LiftV1SBEDecodeError::LiftV1SBEFailedToDecodeTxOutError)?;
...
impl LiftV1 {
    /// Decodes a `LiftV1` from Structural Byte-scope Encoding (SBE) bytes.
    pub fn decode_sbe(bytes: &[u8]) -> Result<LiftV1, LiftV1SBEDecodeError> {
        ...
        if tag[0] != 0x01 {
            return Err(
                LiftV1SBEDecodeError::LiftV1SBEExpectedVariantDiscriminant0x01Error { got: tag[0] },
            );
        }
